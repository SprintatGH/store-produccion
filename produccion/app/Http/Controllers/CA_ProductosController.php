...
use App\CA_ProductosSubcategoria;
use App\CA_ProductosStock;
use App\User;
use File;
...
class CA_ProductosController extends Controller
{
    ...
    public function index()
    {
        ...
        ->join('producto_formato','producto_formato.producto_id','ca_productos.id')
        ...
        ->select('ca_productos.id as id','ca_productos.codigo','ca_productos.estado as estado','ca_productos.avatar as avatar','ca_productos.nombre as nombre','ca_productos.stock_minimo','ca_productos.stock_actual','ca_productos.precio_por_mayor','ca_productos.precio_unitario','ca_productos_subcategoria.nombre as nombre_subcategoria','ca_productos_categoria.nombre as nombre_categoria','formatos.nombre as formatoNombre')
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->nuevoFormato)){
            ...
            {
                $ca_prodForm = new ProductoFormato;
        
                $ca_prodForm->estado = 1;
                $ca_prodForm->empresa_id = session('id_empresa');
                $ca_prodForm->producto_id = $data->id;
                $ca_prodForm->formato_id = $items;
                $ca_prodForm->save();
        ...
        return redirect()->action('CA_ProductosController@index');
    }
