...
@inject('stockFisico', 'App\Modelos\ca\administracion\CA_ProductosStock')
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<p>
						<form method="POST" action="{{ route('CA_productos.reporte_pdf') }}" class="form-horizontal" data-parsley-validate="true">
							@csrf
...
<script>
  ...
  $(document).ready(function () {
  	...
  	function getDataProductos(){
		...
		var route = "{{ url('/cliente/administracion/DTproductos') }}";
		var idioma_espaniol = {
			...
			success: function(data){
				...
				if (data.length > 0)
				{
					 ...
					 for (var i=0; i<data.length; i++){
						 var id = data[i].id;
						 var stockActual = "{{ $stockFisico::stockActual(" + id + ") }}";

						if (data[i].estado==1) {
							var estado = "<a href='#' onclick='cambioEstado(this)' id='In' data-href='/cliente/administracion/productos/" + id + "/0/estado'> <span  class='btn btn-xs btn-white m-b-3'><i class='fa fa-trash t-plus-1 text-danger fa-fw fa-sm'></i></span></a>"
						} else {
							var estado = "<a href='#' onclick='cambioEstado(this)' id='In' data-href='/cliente/administracion/productos/" + id + "/1/estado'> <span class='badge bg-success'>Ac</span></a>"
						}
						

						 if (data[i].avatar){
							var avatarProd = "<img src='/files/productos/" + data[i].avatar + "' class='img-rounded height-30'  />"
					    } else { 
							var avatarProd = "<img src='/files/productos/sin_imagen.png' class='img-rounded height-30' />"
						}
						 info = [
							estado,
