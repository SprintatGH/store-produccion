											...
											<tbody> 
												@foreach ($contenido as $items)
													<tr>
														...
														<td class="with-btn" >
															@if ($items->estado==1)
															<a href="#" onclick="cambioEstado(this)" id="In" data-href="{{route('AD_Empresas.estado',[ $items->id ,0])  }}"> <span  class="btn btn-sm btn-white m-b-10"><i class="fa fa-trash t-plus-1 text-success fa-fw fa-lg"></i></span></a>
																@else
															<a href="#" onclick="cambioEstado(this)" id="In" data-href="{{route('AD_Empresas.estado',[ $items->id ,1])  }}"> <span class="badge bg-success">Ac</span></a>
															@endif
														</td>
														<td> <a href="#" onclick="editar({{$items->id}})"  data-toggle="modal" data-id="{{$items->id}}" data-target="#modal-edit"> {{$items->id}}</a></td>
														<td>{{ $items->nombre }}</td>
														<td>{{ $items->razon_social }}</td>
														<td>{{ $items->email }}</td>
	...
	<div class="modal fade" id="modal-add">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<p>
						<form method="POST" action="{{ route('AD_Empresas.store') }}" class="form-horizontal" data-parsley-validate="true">
							@csrf
